...
parser.add_argument('--noise_sd', default=0.0, type=float,
                    help='standard deviation of Gaussian noise (or range maximum for uniform noise) for data '
                         'augmentation')
...
def main():
    ...
    if args.transform == 'color_shift':
        custom_transform = torchvision.transforms.Lambda(lambda x: color_shift(x, sigma=args.noise_sd))
    elif args.transform == 'hue_shift':
        custom_transform = torchvision.transforms.Lambda(lambda x: hue_shift(x, rand_max=args.noise_sd))
    elif args.transform == 'sv_shift':
        custom_transform = torchvision.transforms.Lambda(lambda x: sv_shift(x, rand_max=args.noise_sd))
    else:
        print('Unrecognized transformation... exiting.')
        quit()

    train_dataset = get_dataset(args.dataset, 'train', custom_transform)
    test_dataset = get_dataset(args.dataset, 'test', custom_transform)
