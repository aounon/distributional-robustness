...
class DatasetGenerator():
    ...
    def _split_validation_set(self, train_portion, train_shuffle=True, train_drop_last=True, random_seed = None):
        ...
        if self.train_data_type == 'ImageNet' or self.train_data_type == 'ImageNetMini' or self.train_data_type == 'TinyImageNet' or self.train_data_type == 'PoisonImageNetMini':
            ...
            datasplit = train_test_split(data, targets, test_size=1-train_portion,
                                         train_size=train_portion, shuffle=True, stratify=targets,random_state=random_seed)
            ...
            datasplit = train_test_split(self.datasets['train_dataset'].data,
                                         ...
                                         shuffle=True, stratify=self.datasets['train_dataset'].targets,random_state=random_seed)
            train_D, valid_D, train_L, valid_L = datasplit
            print('Train Labels: ', np.array(train_L))
            print('Valid Labels: ', np.array(valid_L))
            train_subset.data = np.array(train_D)
    ...
    def _split_train_proxy_validation_set(self, train_portion, proxy_rel_portion, train_shuffle=True, train_drop_last=True, random_seed = None):
        np.random.seed(self.seed)
        train_subset = copy.deepcopy(self.datasets['train_dataset'])
        proxy_subset = copy.deepcopy(self.datasets['train_dataset'])
        valid_subset = copy.deepcopy(self.datasets['train_dataset'])

        assert(self.train_data_type == 'CIFAR10')
        ...
        datasplit_2 = train_test_split(train_subset.data,
                                     train_subset.targets,
                                     test_size=proxy_rel_portion, train_size=1-proxy_rel_portion,
                                     shuffle=True, random_state=random_seed)

        train_D, proxy_D, train_L, proxy_L = datasplit_2

        train_subset.data = np.array(train_D)
        proxy_subset.data = np.array(proxy_D)
        train_subset.targets = train_L
        proxy_subset.targets = proxy_L

        self.datasets['train_subset'] = train_subset
        self.datasets['valid_subset'] = valid_subset
        self.datasets['proxy_subset'] = proxy_subset
        ...
        data_loaders['valid_subset'] = DataLoader(dataset=self.datasets['valid_subset'],
                                                  ...
                                                  shuffle=False, pin_memory=True,
                                                  drop_last=False, num_workers=self.num_of_workers)
        data_loaders['proxy_subset'] = DataLoader(dataset=self.datasets['proxy_subset'],
                                                  batch_size=self.train_batch_size,
                                                  shuffle=False, pin_memory=True,
                                                  drop_last=False, num_workers=self.num_of_workers)
        return data_loaders

    def _split_and_poison_train_proxy_validation_set(self,
        train_portion,
        proxy_rel_portion,
        perturb_tensor_filepath_train_subset,
        perturb_tensor_filepath_proxy_subset,
        perturb_tensor_filepath_valid_subset,
        train_shuffle=True,
        ...
        data_loaders['train_subset'] = DataLoader(dataset=self.poison_dataset(self.datasets['train_subset'], perturb_tensor_filepath_train_subset, patch_location, perturb_type, add_uniform_noise,poison_rate),
                                                  batch_size=self.train_batch_size,
                                                  shuffle=train_shuffle, pin_memory=True,
                                                  drop_last=train_drop_last, num_workers=self.num_of_workers)

        data_loaders['valid_subset'] = DataLoader(dataset=self.poison_dataset(self.datasets['valid_subset'], perturb_tensor_filepath_valid_subset, patch_location, perturb_type, add_uniform_noise,poison_rate),
                                                  batch_size=self.eval_batch_size,
                                                  shuffle=False, pin_memory=True,
                                                  drop_last=False, num_workers=self.num_of_workers)
        data_loaders['proxy_subset'] = DataLoader(dataset=self.poison_dataset(self.datasets['proxy_subset'], perturb_tensor_filepath_proxy_subset, patch_location, perturb_type, add_uniform_noise,poison_rate),
                                                  batch_size=self.train_batch_size,
                                                  shuffle=False, pin_memory=True,
                                                  drop_last=False, num_workers=self.num_of_workers)
        return data_loaders
    def poison_dataset(self, dataset, perturb_tensor_filepath, patch_location='center',perturb_type='classwise',add_uniform_noise=False,poison_rate=1.0):
            perturb_tensor = torch.load(perturb_tensor_filepath, map_location=device)
            if len(perturb_tensor.shape) == 4:
                perturb_tensor = perturb_tensor.mul(255).clamp_(0, 255).permute(0, 2, 3, 1).to('cpu').numpy()
            else:
                perturb_tensor = perturb_tensor.mul(255).clamp_(0, 255).permute(0, 1, 3, 4, 2).to('cpu').numpy()
            dataset.data = dataset.data.astype(np.float32)
            # Check Shape
            target_dim = perturb_tensor.shape[0] if len(perturb_tensor.shape) == 4 else perturb_tensor.shape[1]
            if perturb_type == 'samplewise' and target_dim != len(dataset):
            ...
            poison_samples = collections.defaultdict(lambda: False)
            poison_class = []
            # if poison_classwise:
            #     targets = list(range(0, 10))
            #     if poison_classwise_idx is None:
            #         poison_class = sorted(np.random.choice(targets, int(len(targets) * poison_rate), replace=False).tolist())
            #     else:
            #         poison_class = poison_classwise_idx
            #     dataset.poison_samples_idx = []
            #     for i, label in enumerate(targets):
            #         if label in poison_class:
            #             dataset.poison_samples_idx.append(i)
            # else:
            targets = list(range(0, len(dataset)))
            dataset.poison_samples_idx = sorted(np.random.choice(targets, int(len(targets) * poison_rate), replace=False).tolist())

            for idx in dataset.poison_samples_idx:
                poison_samples[idx] = True
                if len(perturb_tensor.shape) == 5:
                    perturb_id = random.choice(range(perturb_tensor.shape[0]))
                    perturb_tensor_ = perturb_tensor[perturb_id]
                else:
                    perturb_tensor_ = perturb_tensor
                if perturb_type == 'samplewise':
                    # Sample Wise poison
                    noise = perturb_tensor_[idx]
                    noise = patch_noise_extend_to_img(noise, [32, 32, 3], patch_location=patch_location)
                elif perturb_type == 'classwise':
                    # Class Wise Poison
                    noise = perturb_tensor_[targets[idx]]
                    noise = patch_noise_extend_to_img(noise, [32, 32, 3], patch_location=patch_location)
                if add_uniform_noise:
                    noise += np.random.uniform(0, 8, (32, 32, 3))

                dataset.data[idx] = dataset.data[idx] + noise
                dataset.data[idx] = np.clip(dataset.data[idx], a_min=0, a_max=255)
            dataset.data = dataset.data.astype(np.uint8)
            print('add_uniform_noise: ', add_uniform_noise)
            print(perturb_tensor.shape)
            print('Poison samples: %d/%d' % (len(poison_samples), len(dataset)))
            return dataset
