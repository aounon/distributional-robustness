...
parser.add_argument('--noise_shape', default=[20000, 3, 32, 32], nargs='+', type=int, help='noise shape')
...
parser.add_argument('--gaussian_smoothing', action='store_true', default=False)
parser.add_argument('--smoothing_std', default=0.2, type=float, help='smoothing noise')
...
def universal_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV):
    ...
    while condition:
        ...
        iterates += 1
        if args.attack_type == 'min-min':
            condition = error_rate > args.universal_stop_error and iterates < 20
        elif args.attack_type == 'min-max':
            condition = error_rate < args.universal_stop_error and iterates < 20
    return random_noise
...
def samplewise_perturbation_eval(random_noise, data_loader, model, eval_target='train_dataset', mask_cord_list=[]):
    ...
    for i, (images, labels) in enumerate(data_loader[eval_target]):
        ...
        if (args.gaussian_smoothing):
            pred = model(images+torch.randn_like(images)*args.smoothing_std)
        else:
            pred = model(images)
        err = (pred.data.max(1)[1] != labels.data).float().sum()
        loss = torch.nn.CrossEntropyLoss()(pred, labels)
...
def sample_wise_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV):
    ...
    data_loader = datasets_generator._split_train_proxy_validation_set(train_split,proxy_rel_split, train_shuffle=False, train_drop_last=False, random_seed = 1)
    data_loader['train_dataset'] = data_loader['proxy_subset']
    mask_cord_list = []
    idx = 0
    ...
    while condition:
        ...
        for i, (images, labels) in tqdm(enumerate(data_loader['train_dataset']), total=len(data_loader['train_dataset'])):
            ...
            if args.attack_type == 'min-min':
                ...
                perturb_imgs = []
                etas = []
                for i in range(0,images.shape[0],sub_batch):
                    perturb_img, eta = noise_generator.min_min_attack(images[i:min(i+sub_batch,images.shape[0])], labels[i:min(i+sub_batch,images.shape[0])], model, optimizer, criterion, random_noise=batch_noise[i:min(i+sub_batch,images.shape[0])],gaussian_smoothing=args.gaussian_smoothing,smoothing_std=args.smoothing_std)
                    perturb_imgs.append(perturb_img.detach())
                    etas.append(eta.detach())
                perturb_img = torch.cat(perturb_imgs)
                eta = torch.cat(etas)
            elif args.attack_type == 'min-max':
                perturb_img, eta = noise_generator.min_max_attack(images, labels, model, optimizer, criterion, random_noise=batch_noise)
            else:
    ...
    if torch.is_tensor(random_noise):
        ...
        output_noises['proxy'] = new_random_noise
    else:
        output_noises['proxy'] = random_noise
    ...
    for set_name in ['train','valid']:
        random_noise= torch.zeros((round(50000*((1-train_split) if set_name == 'valid' else (train_split * (1-proxy_rel_split)))),3,32,32)).to(device)
        # Search For Noise
        idx = 0
        for i, (images, labels) in tqdm(enumerate(data_loader[set_name + '_subset']), total=len(data_loader[set_name + '_subset'])):
            images, labels, model = images.to(device), labels.to(device), model.to(device)
            ...
            if args.attack_type == 'min-min':
                ...
                etas = []
                for i in range(0,images.shape[0],sub_batch):
                    perturb_img, eta = noise_generator.min_min_attack(images[i:min(i+sub_batch,images.shape[0])], labels[i:min(i+sub_batch,images.shape[0])], model, None, criterion, random_noise=batch_noise[i:min(i+sub_batch,images.shape[0])],gaussian_smoothing=args.gaussian_smoothing,smoothing_std=args.smoothing_std)
                    perturb_imgs.append(perturb_img.detach())
                    etas.append(eta.detach())
                perturb_img = torch.cat(perturb_imgs)
                eta = torch.cat(etas)
            elif args.attack_type == 'min-max':
                perturb_img, eta = noise_generator.min_max_attack(images, labels, model, None, criterion, random_noise=batch_noise)
            else:
                raise('Invalid attack')
        ...
        if torch.is_tensor(random_noise):
            ...
            for idx in range(len(random_noise)):
                ...
                new_random_noise.append(torch.from_numpy(mask))
            new_random_noise = torch.stack(new_random_noise)
            output_noises[set_name] = new_random_noise
        else:
            output_noises[set_name]  = random_noise

    return output_noises
def main():
    # Setup ENV
    datasets_generator = dataset.DatasetGenerator(train_batch_size=args.train_batch_size,
                                                  eval_batch_size=args.eval_batch_size,
    ...
    if args.perturb_type == 'samplewise':
        ...
        if args.use_subset:
            ...
            train_target = 'train_dataset'
    if (args.gaussian_smoothing):
        trainer = Trainer(criterion, data_loader, logger, config, target=train_target,gaussian_noise=args.smoothing_std)
        evaluator = Evaluator(data_loader, logger, config,gaussian_noise=args.smoothing_std)
    else:
    ...
    if args.attack_type == 'random':
        ...
        for key in noise:
            torch.save(noise[key], os.path.join(args.exp_name, key +'_perturbation.pt'))
            logger.info(noise[key])
            logger.info(noise[key].shape)
            logger.info('Noise saved at %s' % (os.path.join(args.exp_name, key +'_perturbation.pt')))
    else:
