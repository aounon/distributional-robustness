...
do
	...
	do
	 export exp_name=${exp_path}/adaptive_sigma_${sigma}
		 python3 perturbation_offline.py --gaussian_smoothing --smoothing_std $sigma --config_path $config_path  --exp_name $exp_name  --version $base_version --train_data_type  $dataset_type --noise_shape 20000 3 32 32 --epsilon  $epsilon  --num_steps $num_steps --step_size $step_size --attack_type $attack_type  --perturb_type $perturb_type  --universal_train_target  $universal_train_target --universal_stop_error $universal_stop_error --norm 2
	done
done
