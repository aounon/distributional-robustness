...
DATASETS = ["imagenet", "imagenet32", "cifar10", "svhn"]
...
def _svhn(split: str) -> Dataset:
    ...
    if split == "train":
        return datasets.SVHN(dataset_path, split='train', download=True, transform=transforms.Compose([
            transforms.RandomCrop(32, padding=4),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor()
        ]))
    elif split == "test":
        return datasets.SVHN(dataset_path, split='test', download=True, transform=transforms.Compose([
            transforms.ToTensor()
        ]))
